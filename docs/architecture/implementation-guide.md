...
apps/
  web/
    app/
      ...
      cart/page.tsx
      checkout/page.tsx
      payments/toss/success/page.tsx
      payments/toss/fail/page.tsx
...
| `orders` | 주문 샘플 10개 이상 |
| `payments` | 성공, 실패, 취소 결제 샘플 포함 |
| `shipments` | 배송 준비, 배송 중, 배송 완료, 배송 지연 샘플 포함 |
...
- 결제 데이터에는 `PAYMENT_PENDING`, `PAID`, `PAYMENT_FAILED` 주문과 `READY`, `APPROVED`, `FAILED`, `CANCELED` 결제 상태를 포함합니다.
- 배송 데이터에는 MCP 지연 감지 데모를 위해 `PREPARING` 상태가 3일 이상 지속된 주문과 `DELAYED` 상태 주문을 포함합니다.
