...
MVP는 실제 백엔드보다 도메인 로직, Mock API, 화면 완성도에 집중합니다.

권장 순서:

1. `packages/domain` 타입과 계산 로직을 확정합니다.
2. `packages/mock-data` 샘플 데이터를 작성합니다.
3. `apps/web` Next.js 앱과 Route Handler를 만듭니다.
4. 고객 상품 탐색 화면을 완성합니다.
5. 장바구니와 주문 생성으로 커머스 흐름을 닫습니다.
6. 같은 리뷰/반품 데이터를 셀러 대시보드에서 다시 해석합니다.
7. 마지막에 QA, 반응형, Empty/Loading/Error 상태를 점검합니다.
...
| Mock Data | `packages/mock-data` | 샘플 사용자, 상품, 리뷰, 쿠폰, 주문, 반품 |
| UI Package | `packages/ui` | 재사용 UI 컴포넌트 |

MVP 이후 검토할 스택은 Prisma, PostgreSQL, NextAuth 또는 자체 인증, 결제 PG SDK, LLM API, S3 호환 스토리지입니다.
...
- 리뷰에는 `heightCm`, `bodyShape`, `purchasedSize`, `fitResult`, `positiveKeywords`, `negativeKeywords`를 포함합니다.
...
- 클라이언트 폼 제출 전 검증합니다.
- API Route Handler에서도 동일한 스키마를 재사용합니다.
