...
이 문서는 고객 웹의 P0 핵심 구매 흐름을 디자인하기 위한 화면별 설계 기준입니다. 제품 범위는 `../product/product-requirements.md`, API와 응답 DTO는 `../architecture/api-reference.md`, 시각 기준은 `ui-guidelines.md`를 따릅니다.
...
- 배송지를 입력하고 장바구니 스냅샷 기준 주문을 만든 뒤 토스페이먼츠 테스트 결제를 진행합니다.
...
- Error: 주문 생성 또는 결제 승인 실패, 재시도 CTA
...
- 주문 생성 후 토스페이먼츠 결제창을 열고, 결제 승인 성공 후 `/orders/:orderNumber`로 이동합니다.
- 내부 `orderId`가 아니라 사용자용 `orderNumber`를 토스 결제 `orderId`로 사용합니다.

### 4.9 주문 완료/상세 `/orders/:orderNumber`

목적:

- 주문이 생성되었음을 확인하고 주문번호, 결제 상태, 배송 상태, 상품, 배송지, 금액을 보여줍니다.
...
- Delivery delayed: 배송 지연 사유와 예상 배송일 안내
